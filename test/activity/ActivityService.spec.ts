...
describe('ActivityService', () => {
    ...
    it('레포지토리에 activity가 없을 경우 papagoApi를 호출한다.', async () => {
        when(await recommendTodoApi.apiCall(anything())).thenReturn(
            response as RecommendTodoApiResponse,
        );
        const sut = new ActivityService(
            instance(repository),
            instance(recommendTodoApi),
            instance(papagoApi),
        );
    ...
    it('레포지토리에 activity가 존재하면 바로 반환한다.', async () => {
        ...
        when(await repository.findOneBy(anything())).thenReturn(
            Activity.create(1, '활동'),
        );
        const sut = new ActivityService(
            instance(repository),
            instance(recommendTodoApi),
